...
function hasCompleteContent(jsonObj, length) {
  const requiredKeys = {
    quick: ['whats_happening', 'why_learned', 'belief', 'pattern', 'practice', 'science'],
    keypoints: ['pattern', 'meaning', 'practice', 'awareness'],
    detailed: ['pattern', 'origin', 'meaning', 'practice', 'science']
  ...
  const keys = requiredKeys[length];
  for (const key of keys) {
    if (!jsonObj[key] || jsonObj[key].trim().length < 10) return false;  // Ensure meaningful content
  }
...
async function callAIService(userFeeling, length, attempt = 1) {
  try {
    ...
    const hasContent = hasCompleteContent(parsed, length);
    
    if ((!isValid || !hasContent) && attempt < 2) {
      console.log('Response invalid or incomplete, regenerating...');
      return await callAIService(userFeeling, length, attempt + 1);
    }
    
    return (isValid && hasContent) ? parsed : FALLBACK_RESPONSES[length];
  } catch (error) {
